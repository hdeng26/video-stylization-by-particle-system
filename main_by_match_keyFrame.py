...
while(cap.isOpened()):
    ...
    keyFlow = np.ones_like(flow)
    if count % 10 == 0:
        key_cap = cv2.VideoCapture('lion.mp4')
        for i in range(count + 10):
            ret3, keyframe = key_cap.read()
            if not ret3:
                print("Can't receive frame (stream end?). Exiting ...")
                break
            keyframe = cv2.GaussianBlur(cv2.cvtColor(keyframe, cv2.COLOR_BGR2GRAY),(5,5),0)

        if ret3 is True:
            keyFlow = cv2.calcOpticalFlowFarneback(prvs, keyframe, None, 0.5, 3, 15, 3, 5, 1.2, 0)
    ...
    for y in range(0, baseOutput.shape[0]):
        for x in range(0, baseOutput.shape[1]):
            #print(flow[y][x][1])
            #print(keyFlow[y][x][1])
            flow[y][x][1] = (keyFlow[y][x][1]/10 + flow[y][x][1])
            flow[y][x][0] = (keyFlow[y][x][0]/10 + flow[y][x][0])
